...
@pytest.mark.parametrize("velocity_setting", [1, 2, 3, 4])
@pytest.mark.parametrize("effort_setting", [1, 2, 3, 4])
@pytest.mark.skipif(int(get_version()[2]) < 5, reason="Simulator version must be 5 or greater")
@pytest.mark.isaacsim_ci
def test_drive_model_constraints(
    sim, num_articulations, device, drive_model, effort_limit_sim, velocity_setting, effort_setting
):
    """Create and implicit actuator with the provided drive model and effort limit."""
    articulation_cfg = generate_articulation_cfg(
        articulation_type="single_joint_implicit",
        drive_model=drive_model,
        effort_limit_sim=effort_limit_sim,
    ...
    velocities = velocity_setting * torch.zeros(
        (articulation.num_isntances, articulation.num_joints), dtype=torch.float, device=device
    )
    articulation.write_joint_velocity_to_sim(velocities)
